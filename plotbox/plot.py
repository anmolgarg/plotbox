...
def simple_plot(data, title=None, x_label=None, y_label=None,
                ylimit=False, abbr_month=False, outfile=None):
    ''' This function plot a titleme series of values'''
    fig = plt.figure(figsize=(16, 9))
    ax = fig.add_subplot(111)
    # Here I have the limit
    if ylimit:
        plt.ylim(0, max(data) + 0.5 * max(data))
    data.plot()
    ...
    # Added part for ticks and month abbreviation
    # Assuming the index is date or datetime with months

    if abbr_month:
        ax.set_xticks(data.index)
        ax.set_xticklabels([x.strftime('%b-%y')
                            for x in data.index], rotation=45)
    else:
        ax.set_xticks(data.index)
        ax.set_xticklabels(data.index, rotation=45)


def durationHistPlot(
        df_duration, title='Bin Duration Ratio', xlabel=None, saveAs=None):
    '''plots the bin or value and the durationRatio of the DataFrame
    produced by ts.make_duration'''
    try:
        binLabels = np.array([str(df_duration.bin[idx]) +
                              ' - ' +
                              str(df_duration.binTop[idx]) for idx in df_duration.index])
        if xlabel is None:
            xlabel = 'Bin'
    except:
        binLabels = np.array([str(df_duration.value[idx])
                              for idx in df_duration.index])
        if xlabel is None:
            xlabel = 'Value'
    ...
    sns.barplot(binLabels, df_duration.durationRatio)
    plt.title(title, size=16)
    plt.ylabel('Time at Value [Normalized]', size=14)
...
def sigHistPlot(pdSeries, binLimit=15, saveAs=None):
    '''plots the istagram of the precentage of occurance of values in a pandas series'''
    nValues = len(pdSeries.unique())

    if nValues <= binLimit:
        counts = pdSeries.value_counts(normalize=True).sort_index()
    else:
        counts = pdSeries.value_counts(
            normalize=True, bins=binLimit).sort_index()
...
def plot_forecast(
        df,
        ylabel='Repairs/Month',
        title='Competing Failure Mode Forecast',
        saveAs=None,
        modes=[],
        plotly=False,
        kind='stacked-bar'):
    '''Renders the plot of a griffin forecast dataframe with plotly or matplotlib'''

    if 'year' in ylabel.lower():
        resample_rule = 'Y'
    elif 'quarter' in ylabel.lower():
        resample_rule = 'Q'
    elif 'month' in ylabel.lower():
        resample_rule = 'M'
    elif 'week' in ylabel.lower():
        resample_rule = 'W'
    elif 'day' in ylabel.lower():
        resample_rule = 'D'

    # group the data by a give list of names
    # also groups like:
    #    if modes = [abc]: columns: [abc 1,abc 2] get summed to [abc]

    if (not modes) == False:
        # from griffin import groupColumns
        df = groupColumns(df, modes)

    df = df.resample(rule=resample_rule, how='sum')
    # print df
    if plotly:
        import cufflinks as cf
        if ('bar' in kind) & ('stack' in kind):
            return df.iplot(kind='bar', barmode='stack', filename=title,
                            world_readable=True, yTitle=ylabel, title=title)
        else:
            return df.iplot(
                kind=kind,
                filename=title,
        ...
        if ('bar' in kind) & ('stack' in kind):
            if 'Actual' in df.columns.tolist():
                df_actual = df[['Actual']]
                df = df[[mode for mode in modes if 'Actual' not in mode]]
                ax = plt.subplot(111)
                df.plot(
                    kind='bar',
                    stacked=True,
                    ...
                    figsize=[
                        ...
                        11],
                    ax=ax)
                plt.plot(range(df_actual.shape[0]), df_actual[
                         'Actual'].tolist(), 'r', label='Actual')
                plt.legend()
                plt.ylabel(ylabel)
                # something like this for plotly with Actual
                # fig = plt.gcf()
                # py.plot_mpl(fig)
            else:
                df.plot(
                    kind='bar', stacked=True, title=title, figsize=[17, 11])
                plt.ylabel(ylabel)
        else:
            df.plot(title=title, figsize=[17, 11])
            plt.ylabel(ylabel)
